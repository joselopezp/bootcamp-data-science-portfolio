# Methodology -- project-6-unsupervised-ml-customer-segmentation

## Framework: CRISP-DM + LEAN

| Phase | Notebook | Key Output |
|-------|----------|-----------|
| 1. Business Understanding | 01 | Problem definition, KPIs, success criteria |
| 2. Data Understanding | 02 | EDA, distribution analysis, missing value audit |
| 3. Data Preparation | 03 | Cleaned + encoded + normalized dataset |
| 4. Modeling | 04 | TBD |
| 5. Evaluation | 05 | TBD |
| 6. Deployment | 06 | Executive report (ES) + notebook documentation |

## LEAN Principles Applied

- **MVA**: Start with the simplest model; increase complexity only when justified
- **Waste elimination**: Only analyze variables decision-relevant to the business question
- **80/20**: Focus on the 20% of analysis that generates 80% of business value
